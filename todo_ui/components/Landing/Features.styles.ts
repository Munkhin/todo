export const featuresStyles = {
  section: "bg-gray-50 py-24",
  container: "mx-auto max-w-7xl px-4",
  title: "text-center text-3xl font-bold md:text-4xl",
  sub: "mt-2 text-center text-lg text-gray-600",
  grid: "mt-16 grid grid-cols-1 gap-8 md:grid-cols-3",
  card: "rounded-lg bg-white p-8 shadow-sm transition-shadow hover:shadow-md",
  iconWrap: "mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-blue-100",
  icon: "h-6 w-6 text-blue-600",
  cardTitle: "mb-3 text-xl font-semibold",
  cardDesc: "leading-relaxed text-gray-600",
}

